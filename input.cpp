...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* y-axis (forward)
		if ( dijs.lY < 0 )					padState[i] |= LEFTSTICK_UP;
		//* y-axis (backward)
		if ( dijs.lY > 0 )					padState[i] |= LEFTSTICK_DOWN;
		//* x-axis (left)
		if ( dijs.lX < 0 )					padState[i] |= LEFTSTICK_LEFT;
		//* x-axis (right)
		if ( dijs.lX > 0 )					padState[i] |= LEFTSTICK_RIGHT;
		//* y-axis rotation (forward)
		if (dijs.lRy < 0)					padState[i] |= RIGHTSTICK_UP;
		//* y-axis rotation (backward)
		if (dijs.lRy > 0)					padState[i] |= RIGHTSTICK_DOWN;
		//* x-axis rotation (left)
		if (dijs.lRx < 0)					padState[i] |= RIGHTSTICK_LEFT;
		//* x-axis rotation (right)
		if (dijs.lRx > 0)					padState[i] |= RIGHTSTICK_RIGHT;
		//* Ａボタン
		...
		//* LSTICKボタン
		if ( dijs.rgbButtons[8] & 0x80 )	padState[i] |= BUTTON_LSTICK;
		//* RSTICKボタン
		if ( dijs.rgbButtons[9] & 0x80 )	padState[i] |= BUTTON_RSTICK;
